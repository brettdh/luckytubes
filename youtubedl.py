...
class FileDownloader(object):
        ...
        calc_percent = staticmethod(calc_percent)
        ...
        calc_eta = staticmethod(calc_eta)
        ...
        calc_speed = staticmethod(calc_speed)
        ...
        best_block_size = staticmethod(best_block_size)
...
class InfoExtractor(object):
        ...
        def suitable(url):
                ...
                return False
        suitable = staticmethod(suitable)
...
#        _VALID_URL = r'^((?:http://)?(?:\w+\.)?youtube\.com/(?:(?:v/)|(?:(?:watch(?:\.php)?)?\?(?:.+&)?v=)))?([0-9A-Za-z_-]+)(?(1).+)?$'
        _VALID_URL = r'^((?:http://)?(?:\w+\.)?youtube\.com/(?:(?:v/)|(?:(?:watch(?:\.php)?)?\?(?:.+&)?v=)))?(?:(?:([0-9A-Za-z_-]+).*)|)$'
...
if __name__ == '__main__':
        try:
                ...
                # Sorry, people in other countries -- Jython doesn't have locale
                # -- swochok
                charset = 'ascii'
                fd = FileDownloader({
